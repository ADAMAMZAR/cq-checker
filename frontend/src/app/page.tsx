...
export default function Dashboard() {
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormSuccessMessage("Successfully saved changes and synchronized Google Sheets!");
  ...
  const getSupplierCosts = () => {
    ...
    evidenceLogs.forEach(ev => {
      ...
      map[name].count += 1;
      map[name].cost += ev.cost_usd || 0.0;
    });
    return Object.values(map).sort((a, b) => b.cost - a.cost);
  };

  const supplierCosts = getSupplierCosts();
  const totalCost = evidenceLogs.reduce((acc, ev) => acc + (ev.cost_usd || 0.0), 0.0);
    ...
    <div className="flex-1 flex flex-col max-w-[1600px] w-full mx-auto p-4 md:p-8">
      ...
      <div className="flex gap-6 mb-8 border-b border-white/5 pb-0.5">
        <button
          ...
          className={`pb-3 px-1 text-sm font-medium tracking-tight border-b-2 transition-all duration-300 ease-out cursor-pointer flex items-center gap-2 hover:text-white active:scale-[0.97] ${
                ...
                <div className="flex gap-2">
                  {(["ALL", "MATCH", "MISMATCH"] as const).map(f => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider transition-all duration-300 ease-out cursor-pointer active:scale-[0.97] ${
                        statusFilter === f 
                  ...
                  filteredLogs.map((log) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-4 rounded-xl border transition-all duration-300 cursor-pointer relative group overflow-hidden ${
                          isSelected 
              ...
              {selectedLog ? (
                <div className="flex-1 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-6 pb-4 border-b border-white/5">
                    ...
                    {selectedLog.total_run_cost_usd && selectedLog.total_run_cost_usd > 0 ? (
                      <div className="text-right">
                        <p className="text-[10px] uppercase font-bold tracking-wider text-gray-500">Execution Cost</p>
                        <p className="text-sm font-bold text-white mt-0.5 tabular-nums">${selectedLog.total_run_cost_usd.toFixed(5)} USD</p>
                      </div>
                    ) : null}
                  </div>
                                    ...
                                    <div className="p-2.5 rounded-lg bg-white/[0.02] border border-white/5 text-[11px] space-y-1.5 font-mono text-gray-400">
                                      ...
                                      {docUsage && docUsage.cost_usd > 0 && (
                                        <>
                                          ...
                                          <div className="flex justify-between">
                                            <span>Est. Cost:</span>
                                            <span className="text-white tabular-nums">${docUsage.cost_usd.toFixed(5)} USD</span>
                                          </div>
                                        </>
                                      )}
                                    </div>
                    ...
                    {isEvidenceLoading ? (
                      ...
                      filteredSuppliers.map((name) => (
                        <div
                          ...
                          className={`px-4 py-2 text-xs text-gray-300 hover:bg-white/5 cursor-pointer transition-colors ${
                            selectedSupplierName === name ? "bg-emerald-600/10 font-medium text-emerald-400" : ""
                          }`}
                        >
                          {name}
                        </div>
                ...
                <form onSubmit={handleSaveForm} className="flex-1 flex flex-col h-full justify-between">
                  <div className="space-y-6">
                    <div className="border-b border-white/5 pb-4">
                      <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/[0.04] border border-emerald-500/10 px-2 py-0.5 rounded-full">
                        OCR Extracted Properties
                      </span>
                      <h3 className="text-lg font-bold text-white mt-1 truncate">{selectedEvidence.filename}</h3>
                      <p className="text-xs text-gray-500">Supplier: <span className="font-semibold text-gray-300">{selectedEvidence.supplier_name}</span></p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Certificate Owner Name</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Issuer Name</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Certificate Type</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Certificate Number</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Effective Date (DD/MM/YYYY)</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Expiration Date (DD/MM/YYYY)</label>
                        <input
                          type="text"
                      ...
                      <div className="md:col-span-2">
                        <label className="text-[10px] uppercase font-semibold text-gray-400 tracking-wider block mb-1">Certificate Location</label>
                        <input
                          type="text"
                  ...
                  <div className="mt-8 border-t border-white/5 pt-4 flex justify-end">
                    <button
                      ...
                      className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold tracking-wide transition-all duration-300 ease-out cursor-pointer active:scale-[0.97] flex items-center gap-2 glow-success disabled:opacity-50"
                    >
                      {isSavingForm ? (
                        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="double-bezel">
              <div className="double-bezel-inner text-center py-6">
                ...
                <h3 className="text-2xl font-black text-white tracking-tight tabular-nums">${totalCost.toFixed(4)} USD</h3>
            ...
            <div className="double-bezel">
              <div className="double-bezel-inner text-center py-6">
                ...
                <h3 className="text-2xl font-black text-white tracking-tight tabular-nums">${avgCost.toFixed(5)} USD</h3>
                      ...
                      supplierCosts.map((sc) => {
                        ...
                        return (
                          <tr key={sc.name} className="hover:bg-white/[0.01] transition-colors duration-300">
                            ...
                            <td className="py-3 px-4 text-right font-mono font-semibold text-white tabular-nums">${sc.cost.toFixed(5)} USD</td>
